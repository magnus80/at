...
namespace USSS.WebPages.B2CPre
{
    internal class ServicesPage
    {
        ...
        public string GoToAvailableSevices(string ban, string db_Ans, string currentTariff, string db_Ms)
        {
            ...
            if (!(new WebElement().ByXPath("//form[@class='profile-balance regular-page-balance']")).Displayed) { return "Не отображены элементы интерфейса: Баланис по номеру"; }
            ...
            var q = @"SELECT s.soc, we.entity_id, wc.category_name
                    ...
                    date + @" 
                        ...
                        and view_ind='Y'
                        and we.template_id = 
                        (select template_id from web_templates where business_type='B2C' and pay_syst_type='PREP' 
                        and entity_type='SOC' and ROWNUM = 1) 
        ...
        private string CheckServices(string ban, string db_Ans, string currentTariff, string db_Ms, string category)
        {
            ...
            var q = @"SELECT s.soc, we.entity_id
                    ...
                    date + @" 
                        ...
                        and view_ind='Y'
                        and we.template_id = 
                        (select template_id from web_templates where business_type='B2C' and pay_syst_type='PREP and ROWNUM = 1' 
                        and entity_type='SOC') 
                        and (NON_PUBLIC_IND in " + status + @") 
                        AND TO_CHAR(S.SALE_EFF_DATE,'YYYYMMDD')<=" + date;
        ...
        public string CheckConnectedServices(string ban, string db_Ans, string db_Ms, string phoneNumber)
        {
            ...
            while (i < servicesQ.Count)
            {
                ...
                if (serviceV.Count == 0)
                {
                    ...
                    {
                        string nam =
                            new WebElement().ByXPath("//div[contains(@id,'" + servicesDb +
                                                     "')]//div[@class='ui-outputpanel ui-widget descr']").Text.Trim(' ');
                        while (nam.IndexOf(' ') != -1)
                        {
                            nam = nam.Remove(nam.IndexOf(' '), 1);
                        }
        ...
        public string ConnectOfService(ref string nameService)
        {
            try
            {
                if (nameService == null)
        ...
        public string SuccessConnectOfService(ref string nameService, ref string number, string db_Ans)
        {
            try
            {
                if (nameService == null)
                {
                    ...
                    if (text.Contains("Запрос") && text.Contains("на подключение услуги") && text.Contains("успешно отправлен."))// && text.Contains("За статусом запроса следите в") && text.Contains("истории заявок"))
                    {
        ...
        public string DisconnectOfService(string nameService)
        {
            try
            {
                if (new WebElement().ByXPath("//div[contains(@class,'switch')][1]//button").Displayed)
                {
                    ...
                    {
                        if (nameService == null)
                            new WebElement().ByXPath("//div[contains(@class,'switch')][1]//button").Click();
                        Thread.Sleep(5000);
                        return "success";
                    }
                    return "Кнопка выбора услуги заблокирована";
